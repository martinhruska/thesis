...
%
In this section, we give an overview of the techniques described in detail in~\cite{habermehl:forest,simacek:thesis}. 
%
...
The transformation into the canonicity-respecting form is discussed in detail in~\cite{habermehl:forest,simacek:thesis}. 
%
...
inductive predicates studied within~\cite{iosif:treewidth,zulleger:entailment}.
%
...
is hence in~\cite{habermehl:forest,simacek:thesis} modified to take into account
such paths as well.
%
...
In~\cite{habermehl:forest,simacek:thesis} we go one step further and define
...
We refer the reader to~\cite{habermehl:forest,simacek:thesis,martin:dp} for the
details.
...
language abstraction} on TAs~\cite{artmc12}, which 
...
We also compare with \predator~\cite{predator11,dudka13}, a~many-times winner of
...
\cite{pale01,Sagiv02,Reynolds:SepLogic:02,InvaderCAV07,indPrSynt07,ndqc07,Zee:pldi08,InvaderCAV08,abduction09,mpq11,slayer11,predator11,overlaid11,indPrSynt07,thor10,sas07:chang_rival_necula},
automata \cite{bhrv06b,lists-counters,deg06}, graph grammars \cite{juggrnaut10,juggrnaut-learning12}, upward closed sets \cite{upwclosure08},
...
% mainly on \cite{habermehl:forest} for the overall overview and on \cite{holik:cexfa,simacek:thesis} for the technical part.
% It does not comprise the works on verification of properties of data stored in forest automata \cite{abdulla:data} and 
...
language abstraction} \cite{artmc12}.
...
for software verification competition \cite{svcompweb} which is anually held on TACAS conference.
Forester is a shape analyser implementing analysis based on forest automata.
The main concepts of this approach were described in detail in the previous chapters
therefore this chapter is going to rather focuse how the improvements were implemented
to the tool.
It will also briefly discuss technical improvements done during the preparations for different editions of competition.

Software verification competition (SV-COMP) is annual competition of verification tools including
sound abstract interpreters or formal verifiers, various static analysers and also unsoud tools
like bounded model checkes.
The competition is based on its own benchmark of the C programs.
The length of programs are usually from tens to hundreds lines of code.

The tools compete in differents categories (chosen by authors of a tool), e.g., reachability,
termination, or memory safety category.
Each category consist of some programs from benchmark and specification defining invariants which are
checked.
E.g., in reachability category there is checked a whether a certain line of program is reached.

A verification tool is supposed for a given program and specification answer whether
the program fullfill specification (tool answers true), break specification (tool answers
false), or tool cannot decided and answers unknown.
...
Forester traditionally competed in the categorie related to heap manipulation.
It was a reachability subcategory containing programs working with dynamic
allocated and memory safety categories.
In memory safety categories, it was checked that there are no invalid memory dereferences,
invalid memory deallocations or memory leaks.  

For each edition of competition a short paper describing a yearly progress was written.
The full text of paper is given in Appendix \ref{app:svcomp}.

\section{Preparing Forester for SV-COMP}
...
so called witness.
The witness format is dialect of GraphML (which is based on XML) defined by competition rules and more detaily described in \cite{fse15-winess}.
Since Forester performs a symbolic execution and creates symbolic states containing link to a previous state
we travel a path from an erroneous state to the beginning of program and gradually generated needed witness in GraphML.

In the following edition, it was needed to adapt Forester to a new competition enviroment.
Particulary, the competition started using Benchexec \cite{benchexec-sttt}, a platform
making easy execution of benchmark over different tools and reporting structured results.
Adaption of Forester meant to implement script called by Benchexec to exece Forester over a given 
verification task.
This scripts another scipt directly encapsulating Forester and translating ouputs to the predefined
one by competition.
It was also needed to create a specification in XML format later uploaded to SV-COMP repository.
...
This improved overall architecture of Forester (and therefore maintability) and make possible
...
was fixed to the next edition (2016) (a possibility of false bug is caused by using abstraction over forest
...
the given state on forest automaton.
It checks whether an intersection of forest automaton from forward run and backward run in given state is empty or not.
If it is empty the found violation is not real.
The intersection of forest automata was also newly implemented for this edition.
After a detection of spurious conterexample, a refinement of abstraction after a detection of spurious counterexample is implemented.
We use predicate abstraction and refinement is done by adding tree automata from forest automaton of backward run to set of predicates.
This makes possible to recover from a detection of a spurious counterexample and subsequently correctly analyse program.
...
This complicates intersection of rest automata since the intersection needs to take semantics of symbols to accout.
...
%
The recent 20 years have seen a rise of many approaches to verification of pointer programs, aka shape analysis, up to their industrial deployment (e.g. the technique of \cite{distefano_biabduction_2011} in Facebook's Infer). 
%
...
%
The dominant position, previously held by frameworks such as \cite{pale,tvla},
is currently occupied by more automated and scalable approaches based on separation
logic (SL)~\cite{reynolds02sl,berdine07cav,rival07} such as symbolic memory
graphs~\cite{dudka13sas}, on forest automata~\cite{habermehl_forest_2012}, and on graph
...
%\lh{myslel jsem effectiveness jako rozhodnutelnost}%
 satisfiability and entailment should be efficiently decidable, as well as additional graph operations needed, e.g., in (higher-order) bi-abduction \cite{distefano_biabduction_2011,loc_shape_2014}.
\item
...
%
The higher-order bi-abduction \cite{loc_shape_2014} is a notable exception: it is capable of learning extremely complex shape invariants such as B+ trees, skip-lists, or threaded trees. 
%
...
%Turning \cite{loc_shape_2014} into a more robust method would require a decidable formalism for reasoning about sets of graphs
...
Outside separation logic, especially the approach based on Forest automata \cite{habermehl_forest_2011,habermehl_forest_2012,holik_boxes_2013,holik_counterexample_17} has been shown viable \cite{baier_forester_2015,holik_run_2016,holik_forester_2017}. It allows for some degree of local reasoning, it is efficient as it allows to utilise advanced algorithms for tree automata (such as simulation reduction or antichain language inclusion). The main distinguishing advantage is its compatibility with abstraction schemes from abstract regular model checking \cite{baier_forester_2015} with counterexample guided refinement loop. The formalism however suffers from some deficiencies, such as that expressible classes shape graphs are limited and that it is not closed under union.
...
First, we explain how graphs can be encoded into trees and tree automata (as a variation on tree decomposition of graphs \cite{Courcelle} and also the formalism used in \cite{CADE24,IosifRogalewiczVojnar:deciding}).
We then discuss basic ideas for an entailment procedure for the formalism.
%
We believe that this new formalism can eventually combine local reasoning of separation logic and forest automata, strong entailment procedures of \cite{Katelaan:seplog,pagel,Iosif:CSL,Iosif:LPAR}, efficiency of tree automata \cite{abdulla_when_2010,almeida_reduction_2016,abdulla_computing_2008,TACAS12}, and powerful abstraction schemes of regular model checking.
...
We will first discuss encoding of graphs as variations on tree decompositions, similar to that used in \cite{CADE24} and also \cite{IosifRogalewiczVojnar:deciding}.
...
The idea for deciding entailment is to use tree automata language inclusion algorithms (such as \cite{abdulla_when_2010,almeida_reduction_2016,abdulla_computing_2008,TACAS12}) over tree automata encodings of tree decompositions.
...
Our long term plan is to develop a shape analysis framework based on this formalism and entailment check in the spirit of \cite{holik_boxes_2013} and also \cite{loc_shape_2014}.
...
%    \end{description}
%  \item \cite{CADE24} CADE Adam, Jirka, Radu
%    \begin{description}
...
%    \begin{description}
%      \item \emph{What?} Seperation logic (SL) was introduced as an extension of Hoare logic for reasoning about mutable data structures by John Reynolds \cite{Reynolds:SL}.
%        Later, it was shown that entailment of SL formulas is generally undecidable \cite{SL-overview} (an overview paper).
%      \item However, there are decidable fragments of SLL. One of them is restriction of SL to reason only about SLL \cite{Ohearn-SLL} (Ohearn paper on SLL) which the paper proved to be
...
The length abstraction overaproximates language of the original automata, i.e.,
