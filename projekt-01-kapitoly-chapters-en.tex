...
E.g., for sake of clarity, we show the main principle on a concrete heap graph
instead of forest automata where one would need to visualise the sets of heap graphs represented by automata.
...
As the first step, all solo boxes, i.e., boxes that are not on a cycle, are unfolded before the abstraction loop is executed.
...
After unfolding the solo boxes, the first abstraction is performed whose result is shown on the right-hand side
...
already shown in the left-hand side of Figure \ref{ref:fig-re1}, i.e., circular doubly-linked lists whose
...
The remaining transition from non-root states contains only the selectors $\mathtt{l,t, r}$ (in this order) what
meas that these states accepts the tree parts of represented heap graph.
...
That respects the fact that in binary tree tree with root pointers, a root pointer always leads
to a root of tree.
The last tree automaton references the first automaton over the selector $\mathtt{n}$ and
the first tree automaton references the last automaton over the selector $\mathtt{p}$ which
models the circularity of doubly-linked lists where the first and the last node are
...
The dotted lines mean that there is a part of automaton pointed by a line which is not presented
here due to space limitations.

Figure \ref{ref:fig-re6.2} contains the same automaton after the first abstraction has been applied.
The abstraction merged states of automaton representing tree part of the heap graph and created
...
\caption{The parts of forest automaton are denoted by red lines as suitable for folding to boxes.}
\label{ref:fig-re7.1}
      \input{figs/running/complex_forest_abstr_folded.tikz}
\caption{The part of forest automaton representing trees are folded to the box.}
\label{ref:fig-re7.2}
\end{center}
\end{figure}
...
\caption{The figure illustrates a box that was detected in the previous forest automaton. This box represents one segment of doubly-linked list.}
\label{ref:fig-re8.1}
      \input{figs/running/complex_forest_dls_folded.tikz}
\caption{The forest automaton after parts of it were folded to the box from the figure above.}
\label{ref:fig-re8.2}
\end{center}
\end{figure}
...
\begin{center}
  ...
  \end{subfigure}
\caption{The cutting points are no longer cutting points after the last folding and can be removed. The forest automaton now consists of one tree automaton.
...
Now the abstraction is performed again but there is nothing to abstract so we continue with illustration of anther
folding done after this abstraction.
The folding algorithm detects that it is possible to fold a sub-graph of doubly-linked list containig two nodes
referencing each other via the $\emph{next,previous}$ selector.
On the forest automata level, there are two root states $\mathtt{b,c}$ and the two root references (one for each root state)
...
It is illustrated in Figure~\ref{ref:fig-re8.1}.

When this pattern is folded to a box named $\mathtt{dls}$ and the corresponding parts of automaton are replaced by the box, we obtain automaton shown in left-hand side
of Figure~\ref{ref:fig-re8.2}.

After the folding we can merge the tree automata in the forest automaton in Figure~\ref{ref:fig-re8.2} to one tree automaton
