...
\begin{figure}[bt]
  ...
  \caption{A scheme of our solution for generating scenarios for orchestrating digital twins based on learning from logs of a real system and then testing MES in an eviroment of digital twin.}
...
Both messages contains simplified description a product built in a hypothetical factory.
...
For each component there is specified a material of the component and a number defining how many pieces of the given component is needed for production.
...
An illustration of our models of the XML message are in Figure \ref{fig:eurocast:models}.
...
Generally, the various atomic values (occuring in XML and JSON messages) correspond to leaf nodes and composite values (lists,
...
The minimum and maximum values keep information about a number of occurences of the given node in the concrete messages of the class
what may be used later during a generation of messages from the abstract representation for testing.
%
...
the XML or JSON format), classify a message to a class and then apply the \emph{group and reduce} function to the message.
The function group the same nodes w.r.t. a given property (usually it groups the nodes with the same keys) in the mesasge to one node what makes the message more concise.
Then we appply the \emph{merge} function to the all messages in the given class which results to
...
If we reduce two nodes with key $a$, one with $30$ maximal occurrences (i.e., the node occurred in $30$ places)
and the second with $20$ maximal occurrences (i.e., the node occurred in $20$ places),
then the resulting node should has $50$ occurrences maximally beucase it represents nodes which occurred in $50$ places at the most.
%
...
The number of minimal and maximal occurrences of the nodes has also changed to $3$ in the top sub-figure 
and to $2$ in the bottom sub-figure which reflect the number of grouped nodes.
...
from the upper tree from Figure~\ref{fig:eurocast:gar}).
...
system after the last event of series. The alphabet $\Sigma$ is obtained by
...
A type of event is determined by communication protocol and the types with the XML and JSON messages are further refined
...
Note, that we can drop some information from the logged message such as the id of communication since
