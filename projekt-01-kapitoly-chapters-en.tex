...
It is important for low level code often deployed in the highly critical
...
The main negatives are the demands on qualification of developers and an inability to scale
to larger software.

There are also automated approaches to proving correctness of program such as
model checking which strive to verify that a program satisfies given specification.
These methods are generally called formal verification.
They are easier to use than manual proving of program, however they have problem with scaling and applicability to real world software.
The are also less general and more specialized automate methods such as different types of sound static analysis or
abstract intepretation which are designed to verify certain program properties
...
This is mostly achieved by static strong type systems which facilitates
encoding different program properties to type system.
...
Moreover, only limited set of program properties is encodable to type systems.
...
They are easy to use, straightforward, scale well but detect many false alarms what can cause
...
writing unit tests or the whole tester teams implementing the complex integration tests.
...
and lightweight automated testing methods which we used in domain where scalability was required.
...
Finite automata are mainly used to represent the various regular stracutres by their languages.
A classical application is language consiting of regular strings (used in pattern matching applications
such as lexical analysis in compilers or detection of malicious strings in security domains).
More sophisticated is language containing regular sequences of messages, or generally objects with an inner structure
what is used, e.g., for modelling communication protocols.
In formal verification, there are also automata representing regular infinite strings (so called Buchi automata)
what is used used for verification of properties such as termination of program.
Another kind of automata used in formal verification are tree automata that represent sets of trees by their languages.
The automata are applied mainly to represent tree data structures allocated on heap by a program.
...
Fortunately, the efficent heuristics for the mentioned operations were introduced making
...
In shape analysis, we are interested in representation of regular data structures allocated on heap such
as linked lists or trees and verification of the properties such as absence of invalid memory
...
languages with manual memory management as C or C++ where memory managment is fully within user's control.
At the same time, these programs are used in critical software such as kernel of operating system.
Any bugs in them may affect many users and lead to the expensive losses.
Therefore they need strong safety quarantees which can the methods with formal basis provide.
...
before MES is deployed to a real manufactory.
...
and rest of manufactory emulated.
...
and enhance it to enable verification of larger class of programs.
We also want to propose a new automata formalism which will be based on Courcelle theorem \cite{Courcelle}
and will be able to represent class of graphs with bound tree width as its language.
...
Our goal is to make Forester able to compete on an international level although we are
...
In other words, we want to make abstraction more precise once a spurious counterexample is found
so when we restart analysis we will no reach the same counterexample again.
...
competitive on international level, by the participations of the Forester tool
...
Although Forester stayed at the state of prototype and a lot of engineering work would be needed to make
...
Finally, the last result in shape analysis was participation on work based on SMT solving.
The possible shapes of heap are described by a points-to relation between pointer variables
and abstract memory objects.
The program is converted to three address code over which a system of equations modelling
operations on heap is put together.
Then the invariants of points-to relation is computed by application of SMT solver to system of equitations.
This approach is more straightforward, more scalable but less generalt than the approaches based on automata.
...
natively ran MES.
Such setup can be used e.g., for testing a new version of MES, when we derive the models
from logs of communication in a real manufactory where an older version of MES was deployed.
Then the new version is run in digital twin and it is checked that it has the same semantics
as the older version where it is expected.

Moreover, we are able to make an abstraction over learnt models and extrapolate the new test
...
The methods described above was implemented in the tool Tyrant \cite{ref_tyrant}.
The tools were developed for testing a particular MES used in industry and is tested
...
First, we give an overview of the formalisms for representing data structures allocated on heap.
...
As we mentioned, \emph{shape analysis} deals with analysis of data structures
...
of selector names.
	  ...
	  \begin{enumerate*}[label=(\alph*)]
	  	\item genericity and expressivity\,---\,the more general the model can represent
                      and therefore more programs can be analyzed,
	  	...
	  	\item efficiency and scalability\,---\,there exist efficient algorithms for manipulation with the model
	  ...
	  abstract-interpretation-based approaches associated with Space Invader \cite{InvaderCAV08}, or \slayer \cite{slayer11}.
          These approaches are, however, are not very general since they are restricted to programs over certain classes of
	  ...
	  in the Infer tool widely used in Facebook \cite{www:fbinfer}.
          ...
          of Hoare triples, i.e., the inference rules are introduced.
          The authors introduce operator $x \not\mapsto$ saying that the pointer $x$ has been deallocated
          (and not reallocated). The operator was needed to distinguish between a dangling pointer and pointer which we do not
          know anything about and it also makes poosible to introduce a frame rule
          for underapproximating reasoning in separation logic.
          The methods were implemented in the tool called Pulse inside Facebook Infer.
	  ...
	  The scalability of the method has not been studied yet but the principles of bi-abduction
	...
	Unfortunately, the logic is undecidable.
        There are different version of graph automata with expressive power equal to MSO, i.e., for
        each MSO formula there is a graph automaton with language (which is set of graphs) equivalent
        to set of models fo the formula.
	However, the undecidability of logic implies that automata have undecidable
...
The presented example should illustate the main principles and give you an intuition,
...
In the figure, we illustrate it be removing the abstracted nodes and keep the root pointers to illustrate
...
bring an intuition how abstraction works, i.e., creates a loop in the graph.
...
for the different editions ofthe  compeitition.
...
perspective to fulfill the requiraments of the competition.
Forester did not have a counterexample analysis or an abstraction refinement for this first participation.
The tool was submitted as a binary file in an archive together
with the scripts wrapping execution for competition.
...
They are manually crafted to capture only certain aspects of analysed program
what makes analyses computationally feasable.
...
application of backward run and counterexample-guided refinement (CEGAR) to shape
...
The second result in shape analysis is the mupltiple participations of the Forester
...
We automatically create test scenarios for digital twins using the learnt modesl
...
We only sketched basic concepts and it is needed to design and prove the algorithm
