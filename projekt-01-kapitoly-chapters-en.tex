...
Fortunately, efficient heuristics \cite{tacas10,bonchi_checking_2013} for the mentioned operations were introduced making
...
for tree automata (in the VATA library) what makes it quiet efficient.
...
\caption{Figure shows circular doubly-linked list and its tree decompositions (from left to right we refer to them as a, b, c, d). In the decompositions (a),
        ...
        Sub-figure~(c) illustrates the reconnection operation. It shows the decomposition obtained by reconnection applied to the right-handed side decomposition of (b)
...
A product is described within \texttt{Item} tags where a name of it is given together with a list of the components needed for a production of the product.  
...
If we reduce two nodes with the key $a$, one with $30$ maximal occurrences (i.e., the node occurred in $30$ places)
...
We use predicate stating that only the nodes with the same children (i.e., their children having
...
In our example, we have types \texttt{REST-1} and \texttt{REST-2} which means that all messages of this type
are communicated via REST protocol but those of the first type are classified to the class $1$
and those of the second type are classified to the class $2$.
...
Backward symbolic execution, what enables distinguishing between real and spurious (caused by abstraction) counterexamples.
...
We implemented the procedure in the tool Tyrant \cite{ref_tyrant} which was supposed to be applied
in the Unis company.
