...
The last approach is based on traditional testing by humans. It may be developers
...
Our approach to shape analysis is used forest automata and automata over graphs
...
using logic formula and so called templates.
The invariant of program under analysis are synthesised using SMT solver.
We designed a new template domain for describing shapes of heap.
Since our domain is designed in template based verification framework we can combine
with already existing templates, e.g., for analysis of integer variables.
...
\item We show how we can build product and power domain combinations
...
In this Chapter, we firstly describe the verification method and the domain for shape
...
We encode the program into a
formula representing a specific \emph{static single assignment form} (SSA).
...
We achive acyclicity of the SSA, we cut the path coming from the end of the loop.
...
\section{Conclusion}
This chapter provided a very brief and dense introduction how we extended the template-based
...
(\texttt{malloc}), \emph{reading through dereferenced pointers}, \emph{memory free} in
our shape analysis to be able to compute shape invariants.
...
E.g., to check dereferences of $\nullptr$, for each expression $*p$
occurring in a program location $i$, we verify the assertion
$\ssa{p}{j} \neq \nullptr$ where $\ssa{p}{j}$ is the version of $p$
valid at $i$.

A detailed description of abstract memory operations or how other properties are checked
can be found in \cite{fmcad18} as well as empirical evaluation of our
method on benchmark of heap manipulating programs.
Their full description of th method is out of scope of this thesis and since
we are not the main authors we limited ourselves to this brief overview.
