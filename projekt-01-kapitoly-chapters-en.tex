...
For a sake of clarity we show the main principle on a concrete heap graph
instead of forest automata where one would need to the sets of heap graphs represented by automaton.
...
\end{center}
\caption{On the left-handed side is the studied data structure with highlighted parts which are going to be folded to a box. On~the right-handed side the highlighted parts of graph are folded and replaced by box \emph{tree-rootptr}.}
\label{ref:fig-re2}
\end{figure}
...
Therefore in this step, abstraction merges all states with the same prefix languages up to height two.
In our case, that are leaves or states with just one children node.
We merge these state to one leave node with the root pointer.
In the figure, we just remove the corresponding nodes and keep the root pointers to illustrate
...
These parts of graph are trees pointed by nodes of doubly-linked list as it is
in Figure~\ref{ref:fig-re2}.
They are folded to box called \emph{tree-rootprt} which is
added to edges leading from nodes of doubly-linked list as shown on the right-handed side of Figure \ref{ref:fig-re2}.
...
The abtraction is shown on Figure \ref{ref:fig-re4}.
...
It finds that the whole heap graph could be folded as it is shown
...
These tasks were split to two two main categories:
\begin{enumerate*}
...
third level) and some test cases from~\cite{vmcai17}, which were a~part of the
...
for different editions of compeitition.
The conceptual improvements of forest auomatata based shape analysis in different
editions of competition are described in the next section.
For the first participation of \forester in SVCOMP (edition 2015) \cite{svcomp15-forester},
no principle changes in Forester were done and we aimed to preparing tool from technical
perspective to fulfill requiraments of the competition.
Forester did not have counterexample analysis or abstraction refinement for this first participation.
...
a found counterexample violating a given specification,
so called witness.
The witness format is dialect of GraphML (which is based on XML) defined by competition rules and described in more detail way in \cite{fse15-witness}.
...
In the next edition of competition (year 2016), it was needed to adapt Forester to a new competition environment.
...
We needed to implement a script called by Benchexec to execute Forester over a given 
verification task.
