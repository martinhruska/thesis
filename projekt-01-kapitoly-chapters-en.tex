...
%
Formally, a trace abstraction up to a length $n$ over an automaton $\A=\aut$ is an equivalence
...
company that offers a~MES as their product.
Particulary, we got the following logs:
\begin{enumerate}
  \item communication between MES and ERP consisting of $228$ XML files where each file is one message,
  \item communication between MES and two machines consisting of $25072$ messages between the first machine and MES and $20$ messages between the second machine and MES,
  \item communication between MES and terminal consisting of $5970$ messages.
\end{enumerate}

That is a total of $31290$ messages.
We processed the logs using the Tyrant tool on machine having $3$ GHz Dual-Core Intel Core i5, $8$ GB DDR3.
We generated a scenario for the Cryton tool having $31303$ steps which corresponds to the
total number of messages with $13$ steps added for initialization of communicatation between MES and ERP.
We generated the scenario in $22$ seconds.

We futher transformed an event calendar representing the scenario to an automaton and applied abstraction over it.
We obtained automaton with $31303$ states (which corresponds to a number of steps in scenario).
After an abstraction we got automaton with $3568$ states and $17$ loops.
We were able to generate the new scenarios using the abstracted automaton.
Although the results of abstraction application look promising,
this feature of Tyrant is still experimental and to be fully aplicable
for testing a particular MES in digital twin it would need some fine tuning.
E.g., some sequences of messages (such as the initial ones) cannot be repeated more than once
or some messages needs to be kept in particular order.
Unfortunately, we did not managed to do this fine tuning in a cooperation with the partner company.

In general, the Tyrant tool is still a prototype and it would need extensive collaboration
