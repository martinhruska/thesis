...
abstract interpretation which are designed to verify certain program properties
...
Finite automata are mainly used to represent the various regular structures by their languages.
A classical application is language consisting of regular strings (used in pattern matching applications
such as lexical analysis in compilers or detection of malicious strings in security domains).
...
Fortunately, the efficient heuristics for the mentioned operations were introduced making
...
Therefore they need strong safety guarantees which can the methods with formal basis provide.
...
allocated on heap.
Then we employ a verification procedure to compute shape invariants for each program location.
In our case, shape invariant is an automaton representing all possible states of heap.
Finally, we check that shape invariants implies that a given specification is fulfilled by program
...
Our approach analyses behavior of software used in a real manufactory, builds its models and then
...
We aim to further development of existing approach to shape analysis based on forest automata
...
Particularly, we focus on \forester \cite{svcompweb} which implements shape analysis for forest automata.
...
observed behaviour of a manufactory with testing in environment of digital twin
...
competitive on international level, by the participation of the \forester tool
...
stating that any graph property definable in monadic second-ordered logic can be decided
...
of selector names.
	  ...
	  and a regular transition relation (represented by transducer or operations over automata) to model the semantics of an analysed system.
          E.g., we use words and trees to represent reachable shapes of heap when ARCM is used for shape analysis.
	...
	Unfortunately, the logic is undecidable.
        ...
        to set of models of the formula.
