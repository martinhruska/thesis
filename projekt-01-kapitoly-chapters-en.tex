...
from the root nodes and then proceeding in the breath-first manner, we obtain
...
The first one is header which includes an timestamp, the identification numbers of a client and a communication, a type message.
...
Then the orchestration continues with the similar three steps.
They have only different arguments which are derived
from the log in Listing~\ref{list:XML2} and from the second messages in Listing~\ref{list:MMM}~\ref{list:TERM} respectively.
...
\end{lstlisting}
\begin{lstlisting}[caption={An example of messages sent between a terminal and MES. The messages consist of a header containing an timestamp, the identification numbers of a client and a communication, a type message, and a body containing data in JSON.},label={list:TERM},frame=tlrb]{TERM}
