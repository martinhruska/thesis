...
against the specification manually.
...
There are also automated approaches to proving correctness of program such as
model checking or automated theorem proving (generally called formal verification)
which strive to verify a program according to given specification.
They are easier to use, however problem with scaling and applicability to real world software.
The lightweighted versions of this kind of methods are different sound static analysis or
abstract intepretation which are specialized to a certain program properties
(e.g., different analyses for values of integer variables and different analyses
for termination properties).
...
encode different program properties to type system.
...
or can be even unusable for certain programmers
...
such as automated testing, unsound static analysis based on patter matching, or bounded model checking.
The methods try to hit sweet spot between mathematical rigorous and plain bug hunting.
They are easy to use, straightforward, scale well but detect many false alarms which may cause
that at the end they are ignored by developers completely.

The last of the main approach is based on traditional testing by humans. It may be developers
...
does not given any guarantees about correctness.

The presented dychotomy does not pretend to be complete or exhaustive.
It tries to summary the main approaches and give an overview of them.
In practice, the different approaches are combined, complement each other and may
fall to different categories.
The proposed thesis focuses on automated formal methods (the first part of thesis)
and lightweight automated methods which we used in domain where scalability was required.
...
Finite automata are (syntactically) simple but powerful tool for representing regular languages. 
...
%protocols).
Since their introduction in 50s by Rabin and Scott \cite{rabin-scott} they have been used in
...
A regular language represented by automaton may contain different regular structures.
A classical example is language consiting of regular strings (used in pattern matching applications
such as lexical analysis in compilers or detection of malicious strings in security domains).
Another possibility is language containing regular sequences of messages (used in modelling communication
protocols).
In formal verification we are intereseted also in representing regular infinite strings (by so called Buchi automata)
what is used used for verification of properties described in linear temporal logic, or in represantation of
regular tree structures (by so called tree automata) what is used to represent tree data structures allocated
on heap by a program.

In the applications of automata in this thesis we are interested in properties such as language inclusion and equivalence
of two finite automata, emptiness or complement of language of finite automaton.
All these properties are decidable but some of them have high complexity complexity,
i.e., complement is NP-complete, language inclusion and equivalence are even PSACE-complete.
Fortunately, there were introduced heuristics for the mentioned operations making
them computationally feasable in many practical cases.

As we mentioned above we focus on application of automata to two different fields.
The first one is shape analysis, i.e., static analysis of programs manipulating
dynamic data structures.
As we mentioned above, language of finite automaton (or its variant) can represent
seto of regular structures.
We are interested in representation of regular data structures allocated on heap such
as linked lists or trees.
...
The error prone programs to the mentioned bugs are written especially in programming
...
(both extensions of tree automata) with bounded tree width.
Both formalisms are able to represent quite wide class of data structures that can be
allocated on heap.
Then we employ a verification procedure to compute shape invariants for each program logcation.
In our case, shape invariant is an automaton representing all possible states of heap.
Finally, we check that shape invariants implies that a given specification is fullfiled by program
(usually checking absence of invalid dereference, invalid free or memory leaks).

The second field of application of automata in this thesis is automated software testing.
Particularly, we focus on testing of manufacturing execution system (MES) in environment of digital twin.
MES is critical software in controlling manufactory where bugs may stop
production or even damage manufacturing machines.
Such bugs may be very expensive and it is important to find as many of them as possible
in MES before it is deployed to a real manufactory.
Our approach analyses behavior of software used in a real manufactury, builds its models and then
generates tests for digital twin of manufactory where, e.g., a new version of software is deployed.

\section{Goals of the Thesis}
The first goal of this thesis is to improve the current state of the art of shape analysis.
We aim to futher development of existing approach to shape analysis based on forest automata
and its improvement to enable verification of larger class of programs.
We also want to propose a new automata formalism which will be based on Courcelle theorem \cite{Courcelle}
and will be able to represent graph with bound tree width as its language.

The second part of the first goal is systematical development of a tool for shape analysis.
Particulary, we focuse on Forester \cite{svcompweb} which implements shape analysis for forest automata.
Our goal is to make Forester able to compete on international level although we are
...
The second goal of thesis is to apply automata to field of testing, particularly
to testing manufacturing execution systems.
We want to create a novel approach combining automated test generation from
observed behaviour of a manufactory with testing in enviroment of digital twin
...
represented state space of program and so enabling representation of infinite
state spaces.
...
Forest automata based shape analysis used an abstraction overapproximating
state of heap, i.e., abstract forest automata represent more possible shapes of heap
than is allocated by program in reality.
However the method did not contain any algorithm to check whether a found counterexample
...
It will be described in more technical way in the rest of the thesis but intuitively it
reverts all actions over forest automata done in forward symbolic execution
starting from forest automaton representing an erroneous configuration.
When forest automata in backward run diverges from the ones in forward run,
it means that a found counterexample is spurious.
...
guarantee that after a restart of verification method we exclude it
from represented state space.

We achieved the second goal in the field of shape analysis, i.e., making a shape analyser
competitive on international level, by the participations of the tool Forester
implementing forest automata based shape analysis
in software verification competition SV-COMP \cite{svcompweb} in edition for years 2015, 2016, 2017, and 2018.
The participation was accompanied by publications \cite{svcomp15-forester,holik_run_2016,holik_forester_2017}
describing the main improvements for the given edition of the competition.
Basically, the improvements are gradual implementation of the new methods devoled for
forest automata based shape analysis described above.
...
The tool also verified soundly some test cases such as skip-list of the 3 level, or
variants of trees which were not analysable by any other tool.
Although Forester stayed at state of prototype and a lot of engineering work would be needed to make
...
We sketch automata using principles from this theorem together with algorithm for entailment
over these automata which has singly exponential complexity.
Automata over graphs with bounded tree width would be able to represent
more general class of graphs than forest automata but still have feasible computational
properties which would make them a suitable domain for shape analysis.

\subparagraph{Automated testing}
The main result in automated testing is a system for orchestrating a digital twin to
...
Moreover, we are able to make an abstraction over learnt models and extrapolate new test
...
shape analysis including a detailed description of backward run and predicate abstraction
over forest automata which was developed as part of the thesis.
...
Chapter \ref{ch:fmcad} describes an alternative approach to shape analysis based on SMT
solving which we participated to develop.
...
First, an overview of the different formalism for representing states of heap is given.
Then the refinement techniques for abstraction over various formalisms are summarized.
Finally, an existing work on automata for graph languages is presented.

\section{Shape Analysis}
As we mentioned, \emph{shape analysis} deals with analysis of data structures allocated on the heap (so called shapes of heap)
by a program.
...
In the following chapter, we abstract data structures allocated on heap to graphs (so called \emph{heap graphs}
or \emph{heap shapes}).
...
of selector names.
	  ...
	  The new value may be needed for an abstraction merging some nodes. E.g., consider
	  ...
	  of heap graphs, and the predicate $\emph{NEXT}: V \times V\rightarrow\{\emph{true}, \emph{false}, \emph{unknown}\}$
          saying that we can go from the~heap node $u$ to the node $V$ using $NEXT$ selector, formally $(u,v)\in E \wedge \alpha(u,v)=NEXT$.
	  ...
	  describing the heaps shapes (even for some list structures).
          Moreover, these approaches are rather fragile. Especially~\cite{indPrSynt07}
	  ...
	  Separation logic has found its way to practice which is demonstrated by its implementation
	  in the Infer tool widely used in Facebook \cite{www:fbinfer}.
          ...
          of Hoare triples. The authors introduce operator $x \not\mapsto$ saying that the pointer $x$ has been deallocated
          (and not reallocated). The operator was needed to distinguish between a dangling pointer and pointer which do not
          know anything about what makes poosible to combine of separation logic with frame rule
          for underapproximating reasoning.
          The methods was implemented in the tool called Pulse inside Facebook Infer.
	  ...
	  The \emph{abstract regular model checking} (ARMC) \cite{artmc} is an automata based method for verification of parametrized and infinite state systems.
	  ...
	  and a regular transition relation (represented by transducer or operations over automata) to model the semantics of an analysed system.
          E.g., we use words and trees to represent reachable shapes of heap when ARCM is used for shape anlysis.
	  ...
	  the work on forest automata-based shape analysis \cite{forester12}.
          The original work required user-provided database of boxes but
          it was later extended to the framework with automatic learning of boxes
          in~\cite{boxes13}.
	  ...
	  A common weakness of the current approaches to shape analysis is a lack of proper support for checking
	  ...
	  refinement.
          ...
          The basic formalism of forest automata used a fixed abstraction \cite{forester12}.
...
may be too abstract we provide with an running example in the following section.
We are going to show computation of fixpoint in abstraction loop where
abstraction and folding are performed alternately until a fixpoint is reached.
For a sake of clarity we show the main principle on the concrete heap graph,
not on forest automata where it would not be possible keep clarity because
of need to visualize the sets of heap graphs represented by automaton.

Consider a data structure allocated on a heap shown on the left-handed side of
Figure \ref{ref:fig-re1}.
It is a doubly-linked list which nodes are also roots of tree with root pointers.
At first, all solo boxes are unfolded before the abstraction loop is executed.
This is illustrated by colored nodes in the figure which were unfolded from boxes.
...
\end{center}
\caption{On the left-handed side is the studied data structure with highlighted parts which are going to be folded to a box. On~the right-handed side is after the folding
which folded the highlighted parts of graph to box \emph{tree-rootptr}.}
\label{ref:fig-re2}
\end{figure}

The first abstraction is performed which result is shown on the right-handed side
...
Therefore in this step, abstraction merges all states in corresponding automaton
that represents tree sub-graphs with depths higher than one.
The resulting automaton automaton now represents doubly linked list which nodes are
trees with depth at least one.

After the first abstraction a folding is performed.
Folding algorithm finds suitable parts of heap graph to be folded.
In our case that are tree parts of shown data structures as it is highlighted on the left-handed side of
Figure \ref{ref:fig-re2}.
These parts of graphs are folded to boxes called \emph{tree-rootprt} which are
added to edges leading from doubly-linked as shown on the right-handed side of Figure \ref{ref:fig-re2}.
...
On the right-handed side is resulting graph after folding which folded the highlighted parts to box \emph{DLS}.}
\label{ref:fig-re3}
\end{figure}

The second abstraction is performed now however the abstraction algorithm finds nothing what could be abstracted.
Then the second folding follows. Folding algorithm finds the suitable sub-graphs for folding which are highlighted by
blue color on the left-handed side of Figure \ref{ref:fig-re4}.
These sub-graphs are segment of doubly-linked list, i.e., part of graph between two DLL nodes.
It folds the sub-graphs to the boxes called \emph{DLS}.
The resulting graph after folding is show on the right-handed side of Figure \ref{ref:fig-re5}.
...
Now, another abstraction is performed over the corresponding automaton
and it merges its states in such way that the automaton now represents
arbitrary doubly-linked list of length at least two.
...
Another folding is performed now.
It finds that the whole heap graph could be folded to a box which is shown
by the left-handed side of Figure \ref{ref:fig-re5}.
...
We provide with an overview of the approach. For full technical description one should check \cite{fmcad18}.

The verification method is based on describing state space of a program w.r.t. to a property of interest
using logic formulae and so called templates.
The templates are speacial logic formulae and conceptually they are similar to abstract domain
in abstract interpretation since they are manually crafted to capture only certain aspects of analysed program
and so they make analyses computationally feasable.
Therefore we call templates (abstract) domains in the of the chapter.
The invariant of program under analysis are synthesised from system of logic formulae and templates
describing state space of program using SMT solver.
...
\item Since we designed our domain in existing framework for template-based
  verification how we can build product and power domain combinations
...
The above equtation would need a solver able to deal with second-order logic quatification,
...
To achive acyclicity of the SSA, we cut the path coming from the end of the loop.
...
The template of our base shape domain has the form of the formula $\templ^S
\equiv \bigwedge_{\ssaplb{i} \in \ptrlb}
...
as a formula $\templ^G_{\ssaplb{i}}(d_{\ssaplb{i}}) \equiv G_{\ssaplb{i}}
\Rightarrow \templ^S_{\ssaplb{i}}(d_{\ssaplb{i}})$ where $G_{\ssaplb{i}}$
is a conjunction of SSA guards associated with the definition of the variable
...
\section{Conclusion}
The previous sections provided a very brief and dense introduction how we extended the template-based
approach to static analysis by shape analysis.
...
our abstract template domain to be able to compute shape invariants.
...
Finally, we specified how to check properties such as \emph{absence of null pointer dereference},
\emph{free safety}, and \emph{absence of memory leaks}.
E.g., to check dereferences of $\nullptr$, we verify the assertion
$\ssa{p}{i} \neq \nullptr$ where $\ssa{p}{i}$ is the version of $p$
valid at $i$ for each expression $*p$ occurring in a program location $i$.
...
we are not the main authors of this approach we limited ourselves to this brief overview.
...
The second project was connecting Symbiotic, a static analyser based on symbolic execution, with
...
over which a symbolic execution is run.
Particularly, Symbiotic does not need to analyse lines of code which were denoted as memory
safe by Predator executed in overapproaximating (sound) mode.
If Predator find a bug in program, Symbiotic takes bug report and combines it with the
results of its static pointer analysis to filter out some spurious counterexamples.
...
bug hunting mode) running on GCC frontend which was a more matured than LLVM fronted
...
for Predator as matured as GCC one which resulted in win of Symbiotic in the Memory Safety category.
