...
\begin{enumerate*}
  ...
  \item improving its tool support, particularly tool \forester, to make it able to compete on an international level.
\end{enumerate*}
...
of selector names.
	  ...
	  are not analysable by any other tool fully automatically.
          ...
          These state machines describe erroneous behaviour in code\,---\,they contain the error states whose
...
To illustrate the proposed application of FAs to shape analysis, this sections provides with a running example.
...
We will first show the main principle on a concrete heap graph at first to build basic intuition
...
\begin{center}
  ...
  \end{subfigure}
\caption{The left-hand side show automata after tree automata has been merged to one since there were no longer any cutting points after the last folding.
...
where the right-handed side decomposition $\dect_0$ from Figure \ref{fig:decomps} (b) is rotated by $\chr(b_0,b_0',\dect_0)$.
...
Templates manually crafted logic formulae to capture only certain aspects of an analysed program
...
controlling terminals, industrial control systems (ICS), enterprise resource planning system.
...
\begin{figure}[bt]
  ...
  \caption{A scheme of our solution for generating scenarios for orchestrating digital twins based on learning from logs of a real system.}
...
Moreover, it does not provide generalization of the same class of messages to one generic syntactic structure. 
...
For each component there is specified a material of component and a number of needed items of the given component for production.
Beside a description of components needed a product production the XML messages contain also name of the product.
...
If we reduce two nodes with key $a$, one with $30$ maximal occurrences (i.e., occurred in $30$ places)
and the second with $20$ maximal occurrences (i.e., occurred in $20$ places),
then the resulting node has $50$ occurrences maximally (i.e., represents nodes which occurred in $50$ places).
%
...
The number of occurrence of the nodes has also changed to $3$ in the left sub-figure 
...
it has $2$ occurrences in the right-hand tree from Figure \ref{fig:eurocast:gar}) and
the maximal number of occurrences is $3$ of the nodes is $3$ (which is the number of the occurrences of the node
...
The messages are not included or shorthanded for the sake of brevity.
The text \texttt{<XML>} or \texttt{<JSON>} means that the whole XML or JSON message would be included in an event.
A type of event is determined by communication protocol and the types of XML and JSON messages are further refined
...
One loop was created in the resulting automaton and a language of the automaton is $(abc)^n$ for $n \geq 1$.
...
Then the orchestration continues with the similar three steps which different only in the arguments (and time).
...
derived from the second message in the in Listing~\ref{list:MMM}), and finally, the sixth step is
...
%
We tested our approach on captured communication provided by a~partner company that offers a~MES as their product. Particularly, we got the following logs:
\begin{enumerate}
...
\begin{enumerate*}
  ...
  \item flexibility (we used automata techniques in a different field, particularly in one part of our work on automated testing).
\end{enumerate*}
...
Moreover, the refinement is informed (i.e., it refined the abstraction in a way needed to exclude
...
published in \cite{FM13} where the library was used as the back end for the tool Z3-noodler\cite{z3_noodler}.
Another paper related to string solving, in which the library is used, was accepted for publication in the OOPSLA 2023 conference.
...
a bug hunting mode) running with a GCC front end, which was more mature than the LLVM fronted
