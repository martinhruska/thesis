...
Consider a data stucture allocated on a heap shown on the left-handed side of
...
Folding alogirthm finds suitable parts of heap graph to be folded.
...
These sub-graps are segment of doubly-linked list, i.e., part of graph between two DLL nodes.
It foldes the sub-graphs to the boxes called \emph{DLS}.
...
artbirtary doubly-linked list of length at least two.
...
encondes whether the program location is reachable.

Wachieve acyclicity of the SSA, we cut the path coming from the end of the loop.
...
\section{Temaple domain for Shape Analysis}
We design a temaplate domain which are used in the described framework to
...
\section{Conclusion}
This chapter provided a very brief and dense introduction how we extended the templated-based
approach to static analys by shape analysis.
...
for shape analysi with others already existing domains.
...
Predator, a sucessfull shape analyser using symbolic memory graphs \cite{svcomp-symbiotic}.
...
The information was further used by Symbiotic which realies heavily on slicing of program
over which a symbolic execution is run.
Particulary, Symbiotic does not need to analyse lines of code which were detoned as memory
