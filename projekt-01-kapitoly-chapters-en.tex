...
them computationally feasible in many practical cases \cite{tacas10,bonchi_checking_2013}.
...
of selector names.
	  ...
	  are not analysable by any other tool fully automatically.
          ...
          The main idead of symbolic execution \cite{soa-king:se} is to run a program under verification on symbolic values instead of concrete ones.
          I.e., the program is run on symbolic input which may have an arbirtary value.
          The concrete program operations from the original program are then replaced by opeartion manipulating symbolic values.
          When a symbolic execution reaches a branching condition, then it continues in the both branches and maintains
          a set of \emph{path conditions} for each brach, i.e., the conditions which needs to be satisfied for the given branch
          ...
          An advatange of this approach is that it does not produce false alarms since all symbolically executed paths are feasible.
          On other hand, a user needs to add \texttt{assert} statements to a source code to represent the error states.
          Symbolic execution may also be computationaly demanding and it may not even terminate on programs
          with unbounded loops or recursion.

          Symbolic execution was sucessfully used to detect bugs in real-world software.
          E.g., The \texttt{KLEE} tool \cite{soa-klee} find bugs in the tools such as \texttt{GNU COREUTILS}.

          In the recent years, the tool Symbiotic \cite{fmics12} based on symbolic execution done by \texttt{KLEE}
          ...
          These state machines describe errorness behaviour in code\,---\,they contain the error states whose
          reachability means that there is a bug in code.
          Once the code is instrumented a program slicer is executed to reduce an input for symbolic execution.
          ...
          invalid dereferences, invalid frees, or memory leaks.
          Futher, it uses a data-flow analysis (particularly, a pointer analysis) to prevent adding
          checks to the manipulations with pointers which are guranteed to be safe by the pointer analysis
          ...
          which would even more reduce complexity of symbolic execution by reducing the unnecessary checks of pointer manipulation. 
...
We start by discussing symbolic execution of C programs in the abstract domain of forest automata.%
...
The trees differ in the nodes which are in the figure highlighted by the different colors.
...
\caption{The same forest automaton after an abstraction is applied. Now the forest automaton represents the same structure but the trees can have arbitrary depth.}
     \label{ref:fig-re6.2}
\end{center}
\end{figure}

To make the running example complete we illustate the same computation on forest automata.
...
This is consistent with the fact that each node of binary tree has a root pointer.
...
It corresponds to the fact that each transition represents a segment of the doubly-linked whose nodes are the roots of a binary tree with root pointers.
More precise would be to draw each transition with two selectors, one labeled by the $\mathtt{dls}$ box and one labeled by the $\mathtt{dls}$ box, but
...
since the represented heap graph has no longer any cut-points (i.e., each node is pointed exatly once) which is shown in the left-hand side of Figure~\ref{ref:fig-re9}.
...
The main weaknesses of \forester came from that it is still a technical immature of the tool.
...
To achieve acyclicity of the SSA, each the path is cutted coming from the end of some loops.
...
However, the main comparative advatenge of our work is combining different analysis,
...
% $root$ key.
%
We illustate a representation of an XML message by our model in the following example.
Consider the XML messages from the Listing \ref{list:XML1} and \ref{list:XML2}.
Both messages contains simplified description of components needed to create a product.
For each component there is specified a material of compoment and a number of needed items of the given component for production.
Beside a description of components needed a product production the XML messages containt also name of the product.
The messages are sent from ERP to MES and they are stored in the files having the timestamps
in their names (particularly, \texttt{20211207-125952.xml} and \texttt{20211207-125955.xml}) in the logs of communication.

An illustration of the models of the XML message are in Figure \ref{fig:eurocast:models}.
The pink nodes are composite nodes and the blue nodes are leaf nodes.
The text inside nodes corresponds to the quadraples in the definition of both kinds of nodes. 
Please note, that the children of the composite nodes are not explicitly in the text but are reflected by the tree
structure of the model.
As one can see the tree structures of the models in the figure corresponds to the structures of the XML models.
...
%
Let us explain why the values $u_i$ and $l_i$ are sumed.
If we reduce two nodes with key $a$, one with $30$ maximal occurences (i.e., occured in $30$ places)
and the second with $20$ maximal occurences (i.e., occured in $20$ places),
then the resulting node has $50$ occurences maximally (i.e., represents nodes which occured in $50$ places).
%
...
We illustrate the \emph{group and reduce} function in the following example.
Consider the models of the messages from Figure \ref{fig:eurocast:models}.
If we apply \emph{group and reduce} to the composite nodes of the models starting
from the root nodes and then proceeding in the breadth-first manner, we obtain
the tree structure represented in Figure \ref{fig:eurocast:gar}.
...
The number of occurence of the nodes has also changed to $3$ in the left sub-figure 
and to $2$ in the right sub-figure which reflect the number of grouped nodes.
...
We illustate the \emph{merge} operation by Figure \ref{fig:eurocast:merge}.
The figure contains a result of application of the operation to the trees from Figure \ref{fig:eurocast:gar}.
The nodes with keys \emph{Name} and \emph{Component} from both trees were merged.
The numbers defining minimal and maximal occurences of nodes \emph{Name} are still $1$ since
the node appeares exactly once in the both reduced trees.
The minimal number of occurences of the nodee with the key \emph{Component} is now $2$ (since
it has $2$ occurences in the right-hand tree from Figure \ref{fig:eurocast:gar}) and
the maximal number of occurences is $3$ of the nodes is $3$ (which is the number of the occurences of the node
in the tree from the left-hand side of the same figure).
...
a~hashing function (particularly, we concatenate string
representing the given values and call a hashing function over the resulting string) over $(\etyp_i,\esend_i,\erec_i)$, i.e., giving away $m_i$ and $\etime_i$,
...
It contains six messages, two between ERP and MES (communicated using REST protocol),
two between terminal and MES (communicated using REST protocol),
and two between machines and MES (communicated using OPC-UA protocol).
The messages between ERP and MES corresponds to the XML messages from Listing~\ref{list:XML1}~and~\ref{list:XML2}
as one can from the timestamps (which are the same as the timestamps in the names of files with XML messages).
The messages between machines and MES are in the Listing~\ref{list:MMM} and the messages between
terminals and MES are in the Listing~\ref{list:TERM}.

Each event contains information as defined above.
The messages are not included or shorthended for the sake of brevity.
The text \texttt{<XML>} or \texttt{<JSON>} means that the whole XML or JSON message would be included in an event.
A type of event is determined by communication protocol and the types of XML and JSON messages are futher refined
by their class (we described concept of a class of a message in the previous section).
In our example, we have types \texttt{REST-1} and \texttt{REST-2} which means that all messages with
this are communicated via REST protocol but those with the first type are classified to the class $1$
and those with the second type are classified to the class $2$.
...
The automaton has seven states and trasition over the created symbols.
Finally if we apply a trace abstraction up to length $3$ we get the automaton on the right-hand side of the figure.
The states $1$ was merged with the state $4$, the state $2$ with the state $5$, and the state $3$ with state $6$.
One loop was created in the resulting automaton and a language of the automaon is $(abc)^n$ for $n \geq 1$.
...
In the following, we will illustrate the transformation of communication logs to a YAML scenario.
...
The messages contains their timestemps. The semantics of message is that
the value of the node $0$ should be set to $99$ (and then to $100$ by the second message) in Machine $001$.
Listing~\ref{list:TERM} shows an example of a log of communcation between terminal and MES consisting of two messages.
The messages have two sections.
The first one is header which includes a timestamp, the identification numbers of a client and a communication, a type of message.
...
Finally, Listing \ref{list:GS} and \ref{list:GS1} shows a~generated scenario consisting of two
...
The third messages is sent three seconds after the start of orchestration from Terminal 001
to MES using REST protocol.
The message contains data from the headers of the logged messages and the data in the JSON format.
Note, that we can drop some information from the logged message such as id of communication since
we get the fresh values of such variables in a simulation in digital twin.

Then the orchestration continues with the similar three steps which different only in the agruments (and time).
The fourth step is communication between ERP and MES containing the XML message from Listing~\ref{list:XML2}.
The fifth step is a message from Machine 001 to MES setting the value of the node with path $0$ to the value $100$ (which is
derived from the second message in the in Listing~\ref{list:MMM}), and finaly, the sixth step is
a message from Terminal 001 to MES containing the JSON message corresponding to the second message in Listing~\ref{list:TERM}.
...
%
We tested our approach on captured communication provided by a~partner company that offers a~MES as their product. Particulary, we got the following logs:
\begin{enumerate}
...
We processed the logs using the Tyrant tool on a machine having a $3$ GHz Dual-Core Intel Core i5 CPU and $8$ GiB DDR3 RAM. We generated a scenario for the Cryton tool having $31,303$ steps, which corresponds to the mentioned $31,290$ messages with $13$ steps added for initialization of the communicatation between the MES and ERP. We generated the scenario in $22$ seconds.

We futher transformed the event calendar representing the above scenario to an automaton and applied abstraction over it. Before the abstraction, we obtained
...
and $17$ loops, and we were able to generate new scenarios using the abstracted automaton.

Although the results of applying the abstraction look promising, this feature of Tyrant is still experimental, and to be fully aplicable for testing a particular
...
once or some messages need to be kept in a particular order. Unfortunately, we did not manage to do this fine-tuning in cooperation with the partner company.
...
\label{ch:concl}
This thesis focused primarly on usage of finite automata in shape analysis
and automated testing.
Our work utilizied the following strong sides of automata in the different contexts:
\begin{enumerate*}
  ...
  \item flexibility (we used autmoata techniques in a different field, particularly in one part of our work on automated testing).
\end{enumerate*}
...
Backward symbolic exection enables distinguishing between real and spurious (caused by abstraction) counterexamples.
...
Although the height abstraction originally used in shape analysis based on forest automata can be refined too, it does not guarantee
an exact exclusion of spurious counterexamples and therefore does not guarantee termination of the verification procedure
unless the height is radically increased, which would have a detrimental effect on the efficiency (note, however,
that the exclusion can be guaranteed using this blind refinement).
...
Moreoer, the refinement is informed (i.e., it refined the abstraction in a way needed to exclude
the given counterexample, not blindy)
enables verification of a new class of data structures
...
Our futher result in automata-based shape analysis is introduction of automata over graphs
...
where the factory is emulated and the MES is deployed natively.
Since we use finite automata to model the communication protocol, we can use abstraction to
overapproximate their languages allowing us ot create new test cases.
...
We implemented the procedure in the tool Tyrant \cite{ref_tyrant} which was supposed to applied
in the Unis company.
...
supoprted it, and so it was not possible to make the real tests of the approach
in practice. 
...
Another feauture of the library should be a user friendliness.
...
interface for Python to allow easy experimentation with the algorithms.
...
published in \cite{FM13} where the library was used as the beckend for the tool Z3-noodler\cite{z3_noodler}.
Another paper related to string solving, in which the library is used, was accpeted for publication in the OOPSLA 2023 conference.

In the field of automated testing of MES-controlled digitalized manufactoring systems,
...
messages in real-life manufactoring systems.
...
was accepted for publication in a book about state-of-the-art verification tools,
...
a bug hunting mode) running with a GCC frontend, which was more mature than the LLVM frontend
...
Finally, the paper based on the library for finite automata described in the previous section was accepted for a publication in the proceedings
of the conference CADE 2023.
...
Our finite automata library was used as the backend of the ENFA tool implementing one of the approaches under the comparison.
The results showed that although some advanced algorithms are fast, such as antichain algorithms and reductions to IC3/PDR,
...
This confirms that a development of the library for finite automata focused on an effecient implementation is a reasonable path of research.
