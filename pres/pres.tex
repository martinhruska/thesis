...
\frametitle{Compatible Form of Hierarchical Forest Automaton}
	...
	\begin{center}
		\includegraphics[scale=0.4]{figs/vmcai/cp.pdf}
	\end{center}
...
\frametitle{Intersection of Hierarchical Forest Automata}
	...
	\end{itemize}
	\begin{center}
		\includegraphics[scale=0.4]{figs/vmcai/is.pdf}
	\end{center}
...
\frame{\frametitle{Forester in SV-COMP}
	\begin{itemize}
	  \item<1-> Forester competed in SV-COMP in years 2015-2018 in the \hlbl{categories related to memory safety} (\emph{MemSafety, HeapReach}).
          \item<2-> Forester has never won any medal, but was able to verify tasks with difficult data structures (skip-lists or various trees) which were not solved by any other tool.
	\end{itemize}
}
%*******************************************************************************
...
\frame{\frametitle{Towards Efficient Shape Analysis with Tree Automata (TA)}
	...
	% \end{itemize}
        % \pause
	\begin{itemize}
        \item We proposed new automata able to represent \hlbl{graphs with a bounded tree-width}.
        \item They are based on tree automata but we work with a single tree automaton instead of tuple of TA (like in forest automata).  
        \item We sketched \hlbl{an efficient algorithm for entailment} for these automata.
        \end{itemize}
}
%*******************************************************************************
...
\frame{\frametitle{Towards Entailment}
	  \begin{itemize}
            ...
            \item<2-> We can create a TA representing all tree decompositions that is of \hlbl{a~polynomial size assuming a fixed tree-width} of the original automaton.
            \item<3-> Since automata TA inclusion is EXPTIME-complete, we have entailment which is \hlbl{singly exponential} (assuming a fixed maximum tree width).
...
\frame{\frametitle{Shape Analysis based on SMT Solving in 2LS Framework}
	\begin{itemize}
          \item Shape analysis uses \hlbl{SMT solving} to compute the points-to relation between pointers and (abstract) memory addresses.
          \item A domain designed for \hlbl{representation of linked-lists}.
          \item Therefore more straightforward than the automata-based approaches, but lacks their generality.
          \pause
...
\frame{\frametitle{Shape Analysis based on SMT Solving in 2LS Framework}
        \begin{itemize}
          \item The verification procedure takes:
	  \begin{itemize}
	  ...
	  \end{itemize}
          \pause
           \item And verifies that there is not a reachable violation of the properties of interest.
           % \item And verifies that no reachable invariants violate the properties of interest.
          \pause
...
\caption{Comparison of 2LS with other tools on examples that combine reasoning about unbounded data structures and their stored data.}
\vspace*{-1mm}
\label{tab:others}
\centering
\scriptsize
...
\frame{\frametitle{Introduction}
  \begin{itemize}
    ...
    \begin{itemize}
      \item A software managing production in factory.
      \item Communicates with information system, machines, and other parts of factory.
  ...
  \end{itemize}
    ...
    \begin{itemize}
      \item Generally, a digital copy of a cyber-physical system\,---\,simulation of reality on computer, often with a graphical interface.
    ...
    \end{itemize}
     ...
     %  \end{itemize}
      \pause
    \item The project solved with the Unis company (\hlbl{MES Pharis}) and Masaryk University (\hlbl{Cryton} for orchestrating digital twin)
    \end{itemize}
}
%*******************************************************************************
...
%*******************************************************************************
\frame{\frametitle{Overview of the Testing Method}
    \begin{center}
...
%*******************************************************************************
\frame{\frametitle{The Tyrant Tool}
\begin{itemize}
  \item Representing messages in the different formats (XML, or JSON) using \hlbl{tree-like data structure}
    \pause
  \item Representing communication using \hlbl{event calendar}.
  \begin{itemize}
    \item Direct generation of a scenario for digital twin.
    \item Transform to finite automaton and apply abstraction to overapproximate language.
    \item New strings in overapproximated language $\Rightarrow$ new series of events $\Rightarrow$ new scenario $\Rightarrow$ \hlbl{new test case}.
  \end{itemize}
  \pause
...
\frame{\frametitle{Conclusion}
	\begin{itemize}
	  ...
	  \begin{itemize}
            ...
            \item A chapter on forest automata in book on software verification \hlbl{with refined presentation of the approach}.
	  ...
	  \end{itemize}
        \pause
          \item Works not presented and only mildly mentioned in the thesis:
	  \begin{itemize}
            \item Connection of Predator and Symbiotic participating in \hlbl{SV-COMP'20}.
            \item The design and implementation of a new efficient automata library called MATA (\hlbl{TACAS'24}).
	  \end{itemize}
	\end{itemize}
}
%*******************************************************************************
...
\frame{\frametitle{Software Products}
        \begin{itemize}
          ...
          \item Fiedor T., \hlbl{Hruška M.}, Smrčka A., Panov S., Rozsíval M., Tureček D., Čeleda P., Pospíšil L. \emph{Tyrant: The Ruler of Digital Twins.} 2021.
	\end{itemize}
}
%*******************************************************************************
...
\begin{frame}[noframenumbering]
	...
	\begin{itemize}
	  ...
	  \begin{itemize}
            \item Context: In conclusion of my thesis, I proposed that one of the future directions is to design an efficient, simple, and user-friendly automata library.
            \item Depends on definition of user-friendliness.
            \item In general, if you work in a small team with limited amount of resources you want a library that has a clear interface and predicatable behaviour, which is a kind of user-friendliness.
            \item For someone wanting to prototype, transparent data structures and algorithms implemented in a manner similar to textbook presentations enhance user-friendliness..
	  \end{itemize}
	\end{itemize}
\end{frame}
%*******************************************************************************
