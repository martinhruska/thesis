...
\frame{\frametitle{Forester in SV-COMP}
	\begin{itemize}
	  \item<1-> Forester competed in SV-COMP in years 2015-2018 in the \hlbl{categories related to memory safety} (\emph{MemSafety, HeapReach}).
          \item<2-> Forester has never won any medal, but was able to verify difficult data structures (skip-lists or various trees) which were not solved by any other tool.
	\end{itemize}
}
%*******************************************************************************
...
\frame{\frametitle{Towards Efficient Shape Analysis with Tree Automata (TA)}
	...
	\begin{itemize}
        ...
        \item They are based on tree automata but we work with a single TA instead of tuple of TA (like in FA).  
...
\frame{\frametitle{Our Solution}
\begin{itemize}
  \item Representing messages in the different formats (XML, or JSON) using \hlbl{Trie} structure (prefix tree)
    \pause
  \item Representing communication using \hlbl{event calendar}.
  \begin{itemize}
    \item Direct generation of a scenario for digital twin.
    \item Transformation to finite automaton\,-\,apply abstraction to overapproximate langauge.
...
\frame{\frametitle{Conclusion}
	\begin{itemize}
	  ...
	  \end{itemize}
        \pause
          \item Work not presented and only mildly mentioned in the thesis:
	  \begin{itemize}
...
\begin{frame}[noframenumbering]
	...
	\begin{itemize}
	  ...
	  \begin{itemize}
            ...
            \item Yes, it can lead to messages or seqeunces of messages that do not make sense but the system under testing should not fail when these messages.
            \item From practical point of view, it is necessary to tune the abstractions to avoid generating such things (common scenario. interesting if CEGAR can be used)
	  \end{itemize}
	\end{itemize}
\end{frame}
%*******************************************************************************
