...
\frame{\frametitle{Introduction}
	\begin{itemize}
			...
			\begin{itemize}
                           \item Shape analysis (most of the work).
                           \item Automated software testing (introduced later).
			\end{itemize}
		\item \hlgr{Shape analysis}\,---\,formal verification of programs with dynamic data structures.
		%\item Tato oblast statické analýzy se soustředí na verifikaci programů pracujících
		%	   s~\hlgr{dynamickými datovými strukturami}.
...
%*******************************************************************************
 ...
 \frametitle{Forest Automata Encoding of Heap}
 	...
 	\begin{itemize}
 			\item A \hlbl{Forest Automaton (FA)}\footnote{\tiny{\textit{P. Habermehl, L. Holik, J. Simacek, A. Rogalewicz, and T. Vojnar. Forest Automata for Verification of Heap Manipulation. FMSD, 41(1):83--106, Springer, 2012.}}}
...
\frame{\frametitle{Forester in SV-COMP}
	\begin{itemize}
	  \item<1-> Forester competed in SV-COMP in years 2015-2018.
          \item<2-> 2015
	  \begin{itemize}
...
\frame{\frametitle{ }
	\begin{center}
		...
		\tiny{Hol{\'{\i}}k, L., \hlbl{Hru\v{s}ka, M.}, Towards Efficient Shape Analysis with Tree Automata. NETYS'21.}
	\end{center}
}
%*******************************************************************************
...
\frame{\frametitle{Towards Efficient Shape Analysis with Tree Automata}
	\begin{itemize}
	  \item<1-> Forest Automata have their limits:
	  \begin{itemize}
	...
	\end{itemize}
        \item<2-> We proposed a new automata capable of representing \hlbl{graphs with bound tree-width}.
          \begin{itemize}
	    \item Particularly, their langauges consist of \hlbl{tree decompositions of graphs with bounded tree width}.
          \end{itemize}
        \end{itemize}
}
%*******************************************************************************
...
\frame{\frametitle{Shape Analysis based on SMT Solving in 2LS Framework}
	\begin{center}
		...
		\tiny{Mal{\'{\i}}k, V., \hlbl{Hru\v{s}ka, M., Schrammel P., Vojnar T.}, Template-Based Verification of Heap-Manipulating Programs. FMCAD'18.}
	\end{center}
}
%*******************************************************************************
...
\frame{\frametitle{Shape Analysis based on SMT Solving in 2LS Framework}
	\begin{itemize}
	  ...
	  \begin{itemize}
            \item Combination with other domains in this Framework, e.g., the numerical domain.
	  \end{itemize}
...
\frame{\frametitle{Shape Analysis based on SMT Solving in 2LS Framework}
	  ...
	  \begin{itemize}
            ...
            \item A set of invariants based on predefined templates (proposed for various domains of data).
            \item The property of interest.
	  \end{itemize}
          \pause
           \item We verify that there is no reachable counterexample violating the invariants.
           \item Technically, i.e., unsatisfaibility of this formula:
	  \begin{itemize}
            \item $\exists \vx, \vxp .\; \neg (\fminit(\vx) \Longrightarrow \templ(\vx, \vd)) \; \vee \neg (\templ(\vx, \vd) \wedge \fmtrans(\vx,\vxp) \Longrightarrow \templ(\vxp, \vd))$
           \item  where $\fminit(\vx)$ is an initial state of variables, $\templ$ represents program properities of interest and negation of their invariants, $fmtrans$ formula representing program
	  \end{itemize}
          \pause
...
\frame{\frametitle{ }
	\begin{center}
    	\large \hlgr{Automated Sotware Testing (with Automata)}

		\vspace{1cm}

		\tiny{Fiedor, T., \hlbl{Hru\v{s}ka, M.}, \hlbl{Smr\v{c}ka, A.}, Generating Scenarios for Digital Twins of Distributed Manufacturing Execution Systems. EUROCAST'22.}
	\end{center}
}
%*******************************************************************************
  ...
  \begin{itemize}
    ...
    \begin{itemize}
     ...
     \item Testing of MES is difficult because:
      \begin{itemize}
       \item \hlrd{Distributed nature of manufacturing}\,---\,different components (ERP, MES, Machines) run it own software and communicate asynchronously.
       \item \hlrd{Different communication protocols} between different components.
       \item \hlrd{Different formats structured data} in messages.
...
%*******************************************************************************
\frame{\frametitle{Context of project}
  \begin{itemize}
	  \item In collaboration with \hlrd{Masaryk University} and \hlrd{UNIS, a.s.} company
          \item The goal of project was to develop digital twin of systems controlled by MES \hlrd{Pharis }developed by UNIS company
          ...
          \item Digital twin was implemented by Masaryk University
  \end{itemize}
}
%*******************************************************************************
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Generic representation and unified manipulation using \hlrd{Trie} structure (prefix tree)
    \item We classify messages \hlrd{based on structural similarity to different classes} and create an abstract representation of each class.
    \item Can automatically generate new messages from abstract representation.
  \end{itemize}
    \pause
  \item Representing model of communication:
  \begin{itemize}
    \item Representing communication in system using \hlrd{event calendar}.
  ...
  \end{itemize}
    \pause
  \item Implemented in the \hlrd{Tyrant} tool.
\end{itemize}
}
%*******************************************************************************
...
\begin{itemize}
  \item Function for transforming event calendar to \hlrd{finite automaton}\,---\,set of events is used as alphabet of automaton
  \item Runs of modeled system (which are basically series of events) are strings in language of automaton
\end{itemize}
\vspace{1em}
\pause
\begin{itemize}
  \item \hlrd{Length abstraction of automaton}\,---\,merges states of automaton with the same prefix language w.r.t. a given length
  ...
  \item Such new strings could be converted back to \hlrd{series of events $\rightarrow$ new scenario $\rightarrow$ new test case}
\end{itemize}
% \vspace{1em}
%\pause
% \begin{itemize}
...
\begin{itemize}
    ...
    \item We \hlrd{automatically} generated a valid scenario for Cryton (so far, the scenarios were created \hlrd{manually} by a human tester only).
    \item We made experiments with automatic new scenarios generation:
    \begin{itemize}
      \item Abstraction only in certain types of communication (Primarly, between MES and machines).
      \item Communication between terminals and MES is not suitable for abstraction (need to preserve sequences of messages).
    \end{itemize}
  \pause
    \item \hlrd{Future Work}
    \begin{itemize}
...
\frame{\frametitle{Conclusion}
	\begin{itemize}
	  ...
	  \begin{itemize}
            \item Connection of Predator and Symbiotic participating in SV-COMP'20.
            \item The design and implementation of efficient automata library (MATA'24).
	  \end{itemize}
	\end{itemize}
}
%*******************************************************************************
...
\frame{\frametitle{Publications}
        \begin{itemize}
          \tiny{\item Chocholaty D., Fiedor T., Havlena V., Holik L., \hlbl{Hruska M.}, Lengal, O., Sic J.: \emph{Mata: A Fast and Simple Finite Automata Library.} In Proc. of TACAS'24.}
          \tiny{\item L Holik, T. Fiedor, A. Rogalewicz, P. Vargovcik, \hlbl{M. Hruska}, J. Sic.: \emph{Reasoning about Regular Properties: A Comparative Study.} In Proc. of CADE'23.}
          \tiny{\item P. Habermehl, L. Holik, \hlbl{M. Hruska}, O. Lengal, A. Rogalewicz, J. Simacek, T. Vojnar. \emph{Forester: Tree Automata in Shape Analysis}. Accepted for publication in the book ASV on the State-of-the-art Tools for Software Verification.}
          \tiny{\item T. Fiedor, \hlbl{M. Hruska}, A. Smrcka. \emph{Orchestrating Digital Twins for Distributed Manufacturing Execution Systems.} In Proc. of EUROCAST'22.}
          \tiny{\item L. Holik, \hlbl{M. Hruska}. \emph{Towards Efficient Shape Analysis with Tree Automata.} In Proc. of NETYS'21.}
          \tiny{\item P. Ayaziova, \hlbl{M. Hruska}, M. Chalupa, M. Jasek, J. Strejcek, V. Sokova, L. Tomovic. \emph{Symbiotic 7: Integration of Predator and More (Competition Contribution).} In Proc. of TACAS'20}
...
\frame{\frametitle{Software Products}
        \begin{itemize}
          \item Chocholaty D., Fiedor T., Havlena V., Holik L., \hlbl{Hruska M.}, Lengal, O., Sic J.: \emph{Mata: A Finite Automata Library.} 2024.
          \item T. Fiedor, \hlbl{M. Hruska}, A. Smrcka, M. Sveda, T. Hradsky. \emph{Analyser of Metrics Measured in Monitoring Center.} 2022.
          \item T. Fiedor, \hlbl{M. Hruska}, A. Smrcka. \emph{Software for measurement and evaluation of performance parameters.} 2021.
          \item T. Fiedor, \hlbl{M. Hruska}, A. Smrcka, S. Panov, M. Rozsival, D. Turecek, P. Celeda, L. Pospisil. \emph{Tyrant: The Ruler of Digital Twins.} 2021.
	\end{itemize}
}
%*******************************************************************************
...
\begin{frame}[noframenumbering]
	...
	\begin{itemize}
		\item A tree decompostion of a graph $G=(V_G,E_G)$ is a tree $T=(V_T,E_T)$, such that:
			\begin{itemize}
